...
bool AABB::intersectsRay(const slm::vec3 &rayOrigin,
                                const slm::vec3 &rayDir,
                                float &t)
{
  // r.dir is unit direction vector of ray
  const slm::vec3 dirfrac( 1.0 / rayDir[0],
                            1.0 / rayDir[1],
                            1.0 / rayDir[2] );
...
{
  return slm::vec3 (orientation[0] > 0.0 ? max().x : min().x,
                    orientation[1] > 0.0 ? max().y : min().y,
...
{
  assert(i>=0 && i<8);
  return slm::vec3 (((i&1)==1 ? max().x : min().x),
                    ((i&2)==2 ? max().y : min().y),
                    ((i&4)==4 ? max().z : min().z));
}
...
{
  return 0.0 == m_min[0] && 0.0 == m_min[1] && 0.0 == m_min[2]
      && 0.0 == m_max[0] && 0.0 == m_max[1] && 0.0 == m_max[2];
}
